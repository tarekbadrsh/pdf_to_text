...
def prepare_messages(prompt: str, content: str, image_path: str = None) -> List[Dict[str, Any]]:
...
def groq_completion(messages: List[Dict[str, Any]]) -> str:
    ...
    completion = groq.chat.completions.create(model=groq_model, messages=messages)
...
def feedback_loop(original_text: str, markdown: str, image_path: str = None) -> str:
    ...
    {markdown}
    """
    messages = prepare_messages(improve_prompt, "Improve the markdown based on feedback", image_path)
    return groq_completion(messages)

def meta_reasoning(original_text: str, markdown_versions: List[str], image_path: str = None) -> str:
    ...
    """
    messages = prepare_messages(prompt, "Perform meta-reasoning and provide optimized markdown", image_path)
    return groq_completion(messages)
...
def process_page(image_path: str) -> str:
    ...
    try:
        ...
        ai_text = ai_read_image(image_path)
        ...
        for i in range(2):  # Perform feedback loop twice
            logger.info(f"Feedback loop iteration {i+1}")
            improved_markdown = feedback_loop(combined_text, improved_markdown)
        
        # Step 4: Meta-Reasoning
        markdown_versions = [initial_markdown, improved_markdown]
        final_markdown = meta_reasoning(combined_text, markdown_versions, image_path)
        
        # Step 5: Create Final Version
        final_version = create_final_version(final_markdown, image_path)
        
        # Save the results
        base_path = image_path.split('.')[0]
        with open(f"{base_path}_groq_optimized.md", "w") as f:
            formatted_final_markdown = format_markdown(final_markdown)
            f.write(formatted_final_markdown)
        with open(f"{base_path}_final.md", "w") as f:
