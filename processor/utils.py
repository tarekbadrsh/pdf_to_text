...
def download_file(
    ...
    if is_valid_url(file_path):
        with aiohttp.ClientSession() as session:
            with session.get(file_path) as response:
                if response.status != 200:
                    raise Exception("Failed to download file")
                with aiofiles.open(local_pdf_path, "wb") as f:
                    f.write(response.read())
    else:
        with aiofiles.open(file_path, "rb") as src, aiofiles.open(
            local_pdf_path, "wb"
        ) as dst:
            dst.write(src.read())
...
def create_selected_pages_pdf(original_pdf_path: str, select_pages: Union[int, Iterable[int]], 
    ...
    with open(original_pdf_path, "rb") as orig_pdf, open(selected_pages_pdf_path, "wb") as new_pdf:
        ...
        if invalid_page_numbers:
            error_message = f"""
            Invalid page numbers: {invalid_page_numbers}
            Input PDF has {total_pages} pages.
